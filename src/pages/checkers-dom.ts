...
const startLocalGameBtn = document.querySelector(
  ".initial-screen .initial-screen-container .container .name-entry #startGameButton"
);
startLocalGameBtn?.addEventListener("click", startLocalGame);

const startAIGameBtn = document.querySelector(
  ".initial-screen .initial-screen-container .container .name-entry #startAIGameButton"
);
startAIGameBtn?.addEventListener("click", startAIGame);

let enforcedJumpCheck: boolean;
const checkBox = document.querySelector(
  ".initial-screen .initial-screen-container .container .name-entry #enforcedJumps"
);
...
const restartLocalGameButton = document.getElementById(
  "restartLocalGameButton"
) as HTMLButtonElement;
const restartAIGameButton = document.getElementById(
  "restartAIGameButton"
) as HTMLButtonElement;

// End of Game elements
const endOfGameSection = document.querySelector(
  ".end-of-game-section"
) as HTMLElement;
const winnerAnnouncement = document.getElementById(
  "winnerAnnouncement"
) as HTMLDivElement;
const playerOneFinalName = document.getElementById(
  "playerOneFinalName"
) as HTMLDivElement;
const playerOneFinalScore = document.getElementById(
  "playerOneFinalScore"
) as HTMLDivElement;
const playerOneFinalCaptured = document.getElementById(
  "playerOneFinalCaptured"
) as HTMLDivElement;
const playerTwoFinalName = document.getElementById(
  "playerTwoFinalName"
) as HTMLDivElement;
const playerTwoFinalScore = document.getElementById(
  "playerTwoFinalScore"
) as HTMLDivElement;
const playerTwoFinalCaptured = document.getElementById(
  "playerTwoFinalCaptured"
) as HTMLDivElement;

// Player One
const playerOneName = document.querySelector(
  ".player-one .container .name"
) as HTMLDivElement;
const playerOneScore = document.querySelector(
  ".player-one .container .score"
) as HTMLDivElement;
const playerOneCaptured = document.querySelector(
  ".player-one .container .captured"
) as HTMLDivElement;
const playerOneTurn = document.querySelector(
  ".player-one .container .turn"
) as HTMLDivElement;

//Player Two
const playerTwoName = document.querySelector(
  ".player-two .container .name"
) as HTMLDivElement;
const playerTwoScore = document.querySelector(
  ".player-two .container .score"
) as HTMLDivElement;
const playerTwoCaptured = document.querySelector(
  ".player-two .container .captured"
) as HTMLDivElement;
const playerTwoTurn = document.querySelector(
...
function startLocalGame() {
  const playerOneName =
    (document.getElementById("playerOneName") as HTMLInputElement).value ||
    "Player 1";
  const playerTwoName =
    (document.getElementById("playerTwoName") as HTMLInputElement).value ||
    "Player 2";
  ...
  (document.querySelector(".initial-screen") as HTMLElement).style.display =
    "none";
    ...
    cols.forEach((col, colIndex) => {
      ...
      if (piece) {
        ...
        if (piece.isKing === true) {
          ...
          pieceDiv.classList.add(
            piece.color === PieceColor.Black ? "black-piece" : "red-piece"
          );
        }
        ...
        pieceDiv.addEventListener(
          "click",
          selectPiece.bind(null, rowIndex, colIndex, pieceDiv)
        );
      }
    });
  });
    ...
    if (moves.length > 0) {
      ...
      moves.forEach((move) => {
        const targetCell = document.querySelector(
          `.col[data-row='${move.endRow}'][data-col='${move.endCol}']`
        ) as HTMLDivElement;
        if (targetCell) {
...
function executeMove(
  ...
  if (piece && piece.color === game.currentPlayer.color) {
    ...
    if (!pieceAtStart && pieceAtEnd) {
      console.log(
        `${piece.color} piece has moved from (${startRow}, ${startCol}) to (${endRow}, ${endCol})`
      );
    }
  }
}
